...
pub fn buildings_page() -> Html {
...

    {
        ...
        use_effect_with((), move |_| {
            wasm_bindgen_futures::spawn_local(async move {
                if let Ok(resp) = reqwasm::http::Request::get("/api/v1/buildings")
                    .send()
                    .await
                {
    ...
    let on_submit = {
        ...
        Callback::from(move |e: SubmitEvent| {
            ...
            let year = year.clone();
            wasm_bindgen_futures::spawn_local(async move {
                let mut req = reqwasm::http::Request::post("/api/v1/buildings")
                ...
                if let Ok(resp) = req.body(payload.to_string()).send().await {
                    if resp.ok() {
                        // Reload list
                        if let Ok(resp2) = reqwasm::http::Request::get("/api/v1/buildings")
                            .send()
                            .await
                        {
