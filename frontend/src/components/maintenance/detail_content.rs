...
#[function_component(MaintenanceDetailContent)]
pub fn maintenance_detail_content(props: &Props) -> Html {
    ...
    let on_add_comment = {
        ...
        Callback::from(move |comment_text: String| {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                ...
                {
                    Ok(_) => {
                        success.set(Some(t("maintenance-comment-posted")));
                        // Reload comments
                        if let Ok(list) = client
                    ...
                    Err(e) => {
                        error.set(Some(t_with_args(
                            "maintenance-failed-post-comment",
                            &[("error", &e.to_string())],
                        )));
                    }
                }
    ...
    let on_delete_comment = {
        ...
        Callback::from(move |comment_id: u64| {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                ...
                {
                    Ok(_) => {
                        success.set(Some(t("maintenance-comment-deleted")));
                        if let Ok(list) = client
                            .get::<Vec<Comment>>(&format!("/requests/{}/comments", request_id))
                            .await
                        {
                    ...
                    Err(e) => {
                        error.set(Some(t_with_args(
                            "maintenance-failed-delete-comment",
                            &[("error", &e.to_string())],
                        )));
                    }
                }
    ...
    let on_upload = {
        ...
        Callback::from(move |form_data: FormData| {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                ...
                {
                    Ok(_) => {
                        success.set(Some(t("maintenance-upload-success")));
                        if let Ok(list) = client
        ...
        <div class="container mt-4">
            ...
            if *loading {
                    ...
                    <div class="col-lg-8 mb-3">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    <h6 class="text-muted small">{t("maintenance-detail-description")}</h6>
                                ...
                                <div class="row small text-muted">
                                    <div class="col-md-6 mb-2">
                                        <strong>{t("maintenance-detail-apartment")}</strong>{" "}{format!("{} ({})", req.apartment_number, req.building_address)}
                                    </div>
                                    <div class="col-md-6 mb-2">
                                        <strong>{t("maintenance-detail-created-by")}</strong>{" "}{&req.created_by_name}
                                    </div>
                                    <div class="col-md-6 mb-2">
                                        <strong>{t("maintenance-detail-assigned-to")}</strong>{" "}
                                        {req.assigned_to_name.as_ref().map(|name| name.clone()).unwrap_or_else(|| t("maintenance-detail-unassigned"))}
                                    </div>
                                    <div class="col-md-6 mb-2">
                                        <strong>{t("maintenance-detail-created")}</strong>{" "}{format_dt_local(&req.created_at)}
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="alert alert-warning">
                    {t("maintenance-request-not-found")}
                </div>
            }
        </div>
    }
